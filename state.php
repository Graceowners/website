...
#################################################################################
$module = "state";
$module_heading = "state(s)";
$table = PREFIX."car_state";
...
if(!empty($_POST) and strtoupper($_SERVER['REQUEST_METHOD'])=='POST'){
    ...
    if(isset($_POST['type']) and $_POST['type']!=''){
        $_post_arrays = [
            ...
            // 'parent'                 =>  clean_string($_POST['parent']),
         // 'short_description'     =>  clean_string($_POST['short_description']),
            'status'                 =>  1,
        ];

        if($_POST['slug']!=''){
            $_post_arrays['slug'] =  create_slug($_POST['slug']);
        }
            ...
            case 'add':
                ...
                if($validation['error']!=true){
                    ...
                    if($duplicate_error['error']==0){
                        ...
                        $resid = $controller->add_new_record($_post_arrays,true);
                        if($resid!=''){
                            $uploads->ID = $resid; 
...
if(isset($_REQUEST['keyword']) and $_REQUEST['keyword']!=''){
    ...
    $args[] =  array('field' => 'slug','operator'=>'%s%','value'=> $ks);
}
